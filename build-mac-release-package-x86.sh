...
rm -rf mac-staging
rm -rf mac-resources
mv "Logisim-Evolution-${VERSION}.pkg" "Logisim-Evolution-${VERSION}-HC-x86.pkg"
...
xcrun altool --notarize-app --primary-bundle-id "$JAVA_APP_IDENTIFIER" \
    ...
    --file Logisim-Evolution-${VERSION}-HC-x86.pkg
...
curl "\$URL" > mac-notarize-x86.log
cat mac-notarize-x86.log

# Check for warnings and errors, then finally:
xcrun stapler staple Logisim-Evolution-${VERSION}-HC-x86.pkg
